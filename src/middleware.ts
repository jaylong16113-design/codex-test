...
const TOOL_PATHS = ['/axiom', '/worker', '/nebula', '/login']

export function middleware(request: NextRequest) {
  const { pathname } = request.nextUrl
  
  // Skip auth for login page itself, api endpoints, homepage, content pages
  const isToolPage = ['/axiom', '/forge', '/blaze', '/hunter', '/mist', '/lens', '/pulse', '/growth', '/cascade', '/compass', '/study'].some(p => 
...
export const config = {
  matcher: ['/axiom/:path*', '/forge/:path*', '/blaze/:path*', '/hunter/:path*', '/mist/:path*', '/lens/:path*', '/pulse/:path*', '/growth/:path*', '/cascade/:path*', '/compass/:path*', '/study/:path*', '/burberry-video/:path*']
}
