...
export default function EnWearPage() {
  ...
  return (
    <div>
      <section className="bg-gradient-to-b from-violet-900/30 to-background px-4 py-12 md:py-16 -mx-4">
        <div className="max-w-3xl mx-auto text-center">
          <div className="text-4xl mb-3">👔</div>
          <h1 className="text-2xl md:text-4xl font-extrabold tracking-tight mb-3 text-white">Men's Style Guide</h1>
          ...
          <div className="flex justify-center gap-2 mt-4 text-xs text-gray-500">
            <span className="tag bg-violet-900/40 text-violet-300">{articles.length} articles</span>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="grid gap-3">
          {articles.flatMap((a, i) => {
            ...
            if (i > 0 && i % 5 === 4) {
              items.push(<AdUnitWrapper key={`ad-${i}`} />);
            }
            return items;
          })}
        </div>
